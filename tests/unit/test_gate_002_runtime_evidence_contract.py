...
def test_runtime_result_files_match_evidence_state_and_support_states():
    approved = "Status: approved" in (ROOT / "docs/decisions/goal-002-runtime-approval.md").read_text(encoding="utf-8")
    for relative in RESULT_FILES:
        content = (ROOT / relative).read_text(encoding="utf-8").lower()
        assert ("status: pass" if approved else "status: not run") in content
        for state in ("not run", "pass", "fail", "blocked"):
            assert state in content


def test_cluster_apply_evidence_matches_permission_state():
    evidence = (ROOT / "reports/gate-002-cluster-apply-and-smoke-summary.md").read_text(encoding="utf-8")
    if "Status: pass; runtime-verified" in evidence:
        assert "Mutation permission granted: yes" in evidence
        assert "Kong runtime applied: yes" in evidence
        assert "Ready for goal 003: yes" in evidence
    else:
