#!/usr/bin/env python3
"""Render the pinned Voice PE firmware configuration for server-side GI wake-up."""
...
def replace_once(source: str, old: str, new: str) -> str:
    """Replace one expected upstream fragment and fail closed on source drift."""
...
def patch_voice_pe_config(source: str, stop_manifest_path: pathlib.Path | None = None) -> str:
    """Pin upstream references and select Home Assistant streaming wake detection."""
    ...
    if sensitivity_count != 1:
        raise ValueError(
            f"expected one Voice PE wake word sensitivity selector, found {sensitivity_count}"
        )
    ...
    source = replace_once(
        source,
        "  # When the voice assistant starts: Play a wake up sound, duck audio.\n  on_start:\n",
        "  # Duck audio only after Home Assistant detects GI.\n  on_wake_word_detected:\n",
    )
    return (
        source.rstrip()
        + "\n\n# Keep credential recovery available without storing Wi-Fi secrets here.\nimprov_serial:\n"
    )


def stage_firmware_assets(output: pathlib.Path) -> None:
    """Copy verified local firmware inputs beside the generated configuration."""
    ...
    for name, expected_sha256 in FIRMWARE_ASSETS.items():
        ...
        if actual_sha256 != expected_sha256:
            raise ValueError(
                f"firmware asset {name} checksum is {actual_sha256}, expected {expected_sha256}"
            )
        shutil.copyfile(source, target_dir / name)


def download_upstream() -> str:
    """Download and verify the exact upstream Voice PE configuration."""
    with urllib.request.urlopen(VOICE_PE_URL, timeout=30) as response:  # noqa: S310
        payload = response.read()
    actual = hashlib.sha256(payload).hexdigest()
    if actual != VOICE_PE_SHA256:
        raise ValueError(f"Voice PE source checksum is {actual}, expected {VOICE_PE_SHA256}")
