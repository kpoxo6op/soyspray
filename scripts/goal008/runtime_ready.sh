...
decision="${repo_root}/docs/decisions/goal-008-runtime-approval.md"
tmp_output="$(mktemp)"
...
run_step "goal008 governance policy apply and smoke" make goal008-governance-policy-apply-and-smoke
run_step "goal004 positive smoke while policy is active" make goal004-security-smoke
run_step "goal004 negative auth behavior while policy is active" make goal004-security-negative-test
run_step "goal008 governance policy rollback and smoke" make goal008-governance-policy-rollback-and-smoke
sleep 2
...

{
  ...
  echo "## Summary"
  echo
  echo "Goal008 runtime acceptance passed locally and is ready for ChatGPT Pro review."
  echo
  echo "## Evidence links"
  echo
  echo "- reports/goal-008-governance-policy-rollout.md"
  echo "- reports/goal-008-governance-policy-rollback.md"
  ...
  echo "## Rollback notes"
  echo
  echo "- Goal008 rollback removes the ValidatingAdmissionPolicyBinding before the ValidatingAdmissionPolicy."
  echo "- Existing Kong runtime resources remain unchanged."
  echo
  echo "## Ready-for-approval statement"
  echo
  echo "Goal008 is ready for Pro approval after evidence is committed and pushed."
} >"${decision}"
