...
decision="${repo_root}/docs/decisions/goal-007-runtime-approval.md"
tmp_output="$(mktemp)"
...
run_step "goal007 consumer onboarding apply and smoke" make goal007-consumer-onboarding-apply-and-smoke
sleep 2
run_step "goal007 consumer onboarding rollback and smoke" make goal007-consumer-onboarding-rollback-and-smoke
sleep 2
...

{
  ...
  echo "## Summary"
  echo
  echo "Goal007 runtime acceptance passed locally and is ready for ChatGPT Pro review."
  echo
  echo "## Evidence links"
  echo
  echo "- reports/goal-007-consumer-onboarding-rollout.md"
  echo "- reports/goal-007-consumer-onboarding-rollback.md"
  ...
  echo "## Rollback notes"
  echo
  echo "- Goal007 rollback removes only the onboarded consumer and its runtime-generated key-auth/ACL credential Secrets."
  echo "- Stable goal004, goal005, and post-goal006-rollback resources remain applied."
  echo
  echo "## Ready-for-approval statement"
  echo
  echo "Goal007 is ready for Pro approval after evidence is committed and pushed."
} >"${decision}"
