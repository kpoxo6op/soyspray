...
curl_status "HTTP smoke result after rollback" 200 --header "apikey: ${BANKLAB_MOBILE_BANKING_APP_API_KEY}"
...
if grep -Eiq '^X-Goal005-Change-Id:' "${tmp_headers}"; then
  fail "temporary header absence: fail"
fi
