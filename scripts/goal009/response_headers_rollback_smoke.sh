...
report="${repo_root}/reports/goal-009-governed-response-headers-rollback.md"
tmp_output="$(mktemp)"
tmp_body="$(mktemp)"
tmp_headers="$(mktemp)"
tmp_rollback="$(mktemp)"
...
write_report() {
  local status="$1"
  {
    echo "# Goal009 Governed Response Headers Rollback Evidence"
    echo
    echo "Status: ${status}"
    echo
    ...
    echo
    echo "Rollback command: make rollback-goal-009"
    echo
    echo "Resources removed or reverted: KongPlugin/tenant-accounts/banklab-goal009-security-headers removed; HTTPRoute/tenant-accounts/banklab-accounts restored to stable goal005 annotation set."
    echo
...
goal009_headers_absent() {
  ! grep -Eiq '^X-Content-Type-Options:' "${tmp_headers}" || return 1
  ! grep -Eiq '^X-Frame-Options:' "${tmp_headers}" || return 1
  ! grep -Eiq '^Referrer-Policy:' "${tmp_headers}" || return 1
  ! grep -Eiq '^X-BankLab-Response-Policy:' "${tmp_headers}" || return 1
}
...
{
  "${python_bin}" scripts/render_goal009_response_headers.py --rollback | kubectl apply -f -
  "${python_bin}" scripts/render_goal009_response_headers.py --plugin-only | kubectl delete --ignore-not-found=true -f -
} >"${tmp_rollback}" 2>&1 || {
  cat "${tmp_rollback}" >&2
  write_report "fail"
  exit 1
...
wait_for_goal009_headers_absent() {
  local status
  for _ in $(seq 1 30); do
    ...
    if [[ "${status}" == "200" ]] && goal009_headers_absent; then
      return 0
    fi
    sleep 2
  done
  fail "goal009 response header absence: fail; last_status=${status:-unknown}; body=$(cat "${tmp_body}")"
}
...
echo "goal009 response headers absent after rollback: pass" | tee -a "${tmp_output}"
...
if [[ "${plugins}" == *"banklab-goal009-security-headers"* ]]; then
  fail "runtime route annotation still includes goal009 plugin after rollback"
fi
echo "runtime route annotation omits goal009 plugin after rollback: pass" | tee -a "${tmp_output}"

if kubectl -n tenant-accounts get kongplugin banklab-goal009-security-headers >/dev/null 2>&1; then
  fail "goal009 KongPlugin still exists after rollback"
fi
echo "goal009 KongPlugin resource absent after rollback: pass" | tee -a "${tmp_output}"
