"""Test the Wyoming path from Piper to the GI detector."""
...
async def describe_models() -> set[str]:
    """Get the wake model names from the GI service."""
...
async def main() -> None:
    """Detect GI and reject the listed non-GI phrases."""
    models = await describe_models()
    if models != {MODEL}:
        raise RuntimeError(f"GI service listed unsupported wake models: {models}")
    ...
    for names in FALLBACK_REQUESTS:
        ...
        if not NotDetected.is_type(event.type):
            raise RuntimeError(
                f"GI service accepted unsupported model names {names!r}: {event.type}"
            )

    print(f"GI wake test passed: detected {detection.name} and rejected the other phrases")
