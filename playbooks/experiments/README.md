# Experiments

Experimental playbooks and proof-of-concepts.

## Purpose

This directory contains experimental work, proof-of-concepts, and temporary playbooks that are not yet ready for production use.

## Structure

- README.md - Experiment documentation
- assets/ - Supporting files and images

## Guidelines

- Experiments should be clearly documented
- Include README.md explaining purpose and status
- Keep experiments isolated from production playbooks
- Move to appropriate location when ready for production

