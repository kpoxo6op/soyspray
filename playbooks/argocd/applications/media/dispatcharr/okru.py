...
Vendored from Streamlink 8.5.0 under the BSD-2-Clause license. The two
validate.any expressions accept both string and object metadata returned by OK.
"""
...
class OKru(Plugin):
    ...
    def _get_streams(self):
        ...
        schema_metadata = validate.Schema(
            validate.any(dict, validate.parse_json()),
            {
                validate.optional("author"): validate.all(
                    {validate.optional("name"): str}, validate.get("name")
                ),
        ...
        metadata, metadata_url = self.session.http.get(
            ...
            schema=validate.Schema(
                ...
                {
                    "flashvars": {
                        validate.optional("metadata"): validate.any(dict, str),
        ...
        return {
            f"{self.QUALITY_WEIGHTS[name]}p" if name in self.QUALITY_WEIGHTS else name: HTTPStream(
                self.session, url
            )
