...
- A LoadBalancer `Service` on `192.168.20.31:9696`

## Access

After ArgoCD sync:

- Visit <http://192.168.20.31:9696/> to configure Prowlarr
...
curl -X POST \
    ...
    http://192.168.20.31:9696/api/v1/indexer
```
...
curl -s -H "X-Api-Key: 7057f5abbbbb4499a54941f51992a68c" \
  http://192.168.20.31:9696/api/v1/indexer | jq '.[] | {id: .id, name: .name, enabled: .enable}'
```
