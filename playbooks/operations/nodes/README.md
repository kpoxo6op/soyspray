...
The dry-run validates the survivor cluster, retained host and filesystem, and
storage preparation. Repository CI validates the full playbook syntax.
Kubespray cannot reliably simulate a fresh host because later tasks depend on
binaries and systemd units that earlier check-mode tasks only predict. The
apply run prepares the existing `/storage` filesystem without formatting, runs
