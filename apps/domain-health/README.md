...
keeps its manifests in `manifests/` and source in `app/`. The exporter runs from
a pinned GHCR image. Source edits build an image and open a separate digest
promotion; they do not change the running exporter. Service, Deployment,
namespace, and provider identities are preserved.
...
promotion changes only the digest. Runtime command or mount overrides stop the
promotion for a separate configuration review. Environment Secret references,
selectors, resources, ports, probes, and container restrictions stay unchanged.
