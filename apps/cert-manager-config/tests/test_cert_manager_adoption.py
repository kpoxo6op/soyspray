...
LEGACY = yaml.safe_load(
    (
        APP.parents[1]
        / "playbooks/argocd/applications/infrastructure/cert-manager/cert-manager-application.yaml"
    ).read_text()
)["spec"]
...
def test_adoption_requires_known_idle_app_and_preserves_foreign_finalizers(change):
    ...
    variables = {
        **PLAY["vars"],
        "cert_manager_existing_app": {"resources": [app]},
        "cert_manager_legacy_spec": LEGACY,
        "cert_manager_native_spec": SPEC,
    }
    allowed = True
