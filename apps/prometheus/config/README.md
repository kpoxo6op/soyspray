...
The `prometheus-config` Argo child owns the Soyspray Grafana dashboard
ConfigMaps and custom PrometheusRule objects in `monitoring`. Edit the JSON in
`dashboards/` or the rules in `alerts/`, then merge to `main`. The child updates
stable dashboard ConfigMap names and prunes removed files, so a Git revert
also removes a temporary dashboard or rule. The Prometheus stack and CRDs have
separate, non-pruning owners.
