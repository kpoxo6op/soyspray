...
The vendored [official v0.15.0 manifest](manifests/manifest.yaml) is unchanged. Its
controller image is pinned in `manifests/kustomization.yaml`. Its sidecar
...
```sh
kubectl kustomize apps/cnpg-operator/manifests
python3 apps/cnpg-operator/manifests/tests/test_plugin.py
```
