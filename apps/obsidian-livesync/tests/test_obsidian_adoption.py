...
LEGACY = yaml.safe_load(
    (
        APP.parents[1]
        / "playbooks/argocd/applications/database/obsidian-livesync/obsidian-livesync-application.yaml"
    ).read_text()
)["spec"]
...
def test_adoption_requires_known_idle_app_and_verified_original_storage(change):
    app = {
    ...
    volume = {
        "spec": {
            "claimRef": {
                "uid": "original",
                "name": "obsidian-livesync-couchdb-rescue-longhorn",
                "namespace": "obsidian",
            },
            "csi": {"driver": "driver.longhorn.io", "volumeHandle": "original-pv"},
        }
    }
    if change == "native":
        ...
        claim["metadata"]["uid"] = "replacement"
    elif change == "wrong-volume":
        volume["spec"]["claimRef"]["uid"] = "another-claim"
    elif change == "wrong-driver":
        volume["spec"]["csi"]["driver"] = "another-driver"
    variables = {
        **PLAY["vars"],
        "obsidian_livesync_existing_app": {"resources": [app]},
        "obsidian_livesync_legacy_spec": LEGACY,
        "obsidian_livesync_native_spec": SPEC,
        "obsidian_adoption_claim": {"resources": [claim]},
        "obsidian_adoption_volume": {"resources": [volume]},
    }
    if change != "missing-proof":
        variables["obsidian_adoption_claim_uid"] = "original"
    allowed = True
