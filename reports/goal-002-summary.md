# Goal: goal-002-kong-oss-baseline

Status: pass; runtime-verified

Branch: kong-goals-foundation

Generated at: 2026-07-06T21:24:18+12:00

## Objective Summary

Implement the local Kong OSS baseline layer with pinned versions, DB-less mode,
Gateway API smoke routing, private Admin API exposure, tests, documentation,
and evidence.

## Goal 001 Approval Status

- ChatGPT Pro approved goal 001 to proceed based on the reported evidence.

## Goal 001 Corrections Reviewed

- Goal 001 evidence remains in `reports/goal-001-summary.md`.
- Booklore and Immich default/plain DB secret values remain placeholder-only.
- CI remains cluster-free.
- Optional goal 001 cluster checks remain not run in this local-only goal 002
  implementation.

## Corrections Applied

- Goal 001 validators now exclude goal 002 Kong resources from before-Kong
  checks.
- Goal 000 foundation validation now preserves the OSS boundary instead of
  forbidding the goal 002 Kong subtree.
- Goal 002 static validation rejects Enterprise images/features, Admin API
  external exposure, floating tags, forbidden plugins, and real secrets.
- Gateway API is pinned to `v1.3.0` for KIC `3.5.10` compatibility according to
  Kong's version compatibility matrix.

## Selected Versions

- Kong Gateway image: `kong`
- Kong Gateway tag: `3.9.3`
- Kong Ingress Controller image: `kong/kubernetes-ingress-controller`
- Kong Ingress Controller tag: `3.5.10`
- Kong Helm chart: `kong/ingress`
- Kong Helm chart version: `0.24.0`
- Gateway API channel: `standard`
- Gateway API version: `v1.3.0`

## Validation Commands Run

- `make validate`: pass
  - Last output line: `Repository foundation validation passed.`
- `make validate-yaml`: pass
  - Last output line: `YAML validation passed for 77 files.`
- `make validate-kustomize`: pass
- `make validate-prereqs`: pass
  - Last output line: `Platform prerequisite validation passed.`
- `make validate-kong-baseline`: pass
  - Last output line: `Kong baseline validation passed.`
- `make render-kong-baseline`: pass
- `make kong-static-test`: pass
  - Last output line: `============================== 12 passed in 0.23s ==============================`
- `make kong-admin-exposure-test`: pass
  - Last output line: `Kong baseline validation passed.`
- `make test`: pass
  - Last output line: `============================== 44 passed in 1.00s ==============================`
- `make policy-test`: pass
  - Last output line: `============================== 22 passed in 3.55s ==============================`
- `make docs`: pass
  - Last output line: `INFO    -  Documentation built in 0.45 seconds`
- `make evidence-goal-002`: pass
  - Last output line: `reports/goal-002-summary.md generated by this command.`

## Runtime Commands Run

- `make cluster-smoke`: pass
- `make cluster-prereq-smoke`: pass
- `make kong-install-dry-run`: pass
- `make kong-apply`: pass
- `make kong-cluster-smoke`: pass
- `make kong-route-smoke`: pass
- `make kong-admin-exposure-test`: pass
- Unknown-route negative smoke: pass, HTTP `404`
- Admin API LoadBalancer port 8444 probe: pass, connection failed as expected
- `make kong-rollback`: not run

## Created/Updated Files

- `.editorconfig`
- `.github/workflows/ci.yml`
- `.gitignore`
- `.pre-commit-config.yaml`
- `CODEOWNERS`
- `Makefile`
- `README.md`
- `ROADMAP.md`
- `apis/README.md`
- `apis/api-template/README.md`
- `apis/api-template/openapi.yaml`
- `apis/api-template/ownership.yaml`
- `apis/api-template/tests/README.md`
- `docs/adr/0001-platform-direction.md`
- `docs/adr/README.md`
- `docs/adr/adr-template.md`
- `docs/architecture/README.md`
- `docs/architecture/certificate-management.md`
- `docs/architecture/gateway-api-baseline.md`
- `docs/architecture/gitops-bootstrap.md`
- `docs/architecture/home-lab-networking.md`
- `docs/architecture/kong-admin-api-exposure.md`
- `docs/architecture/kong-baseline-testing.md`
- `docs/architecture/kong-oss-baseline.md`
- `docs/architecture/network-policy-baseline.md`
- `docs/architecture/operating-model.md`
- `docs/architecture/oss-vs-enterprise.md`
- `docs/architecture/platform-prerequisites.md`
- `docs/architecture/platform-principles.md`
- `docs/architecture/secrets-management.md`
- `docs/architecture/testing-strategy.md`
- `docs/decisions/README.md`
- `docs/index.md`
- `docs/onboarding/README.md`
- `docs/onboarding/api-onboarding-template.md`
- `docs/runbooks/README.md`
- `docs/runbooks/kong-admin-api-exposure-check.md`
- `docs/runbooks/kong-baseline-install.md`
- `docs/runbooks/kong-baseline-rollback.md`
- `docs/runbooks/network-policy-recovery.md`
- `docs/runbooks/platform-prereqs-rollback.md`
- `docs/runbooks/runbook-template.md`
- `mkdocs.yml`
- `platform/README.md`
- `platform/bootstrap/README.md`
- `platform/bootstrap/apply-prereqs.example.sh`
- `platform/bootstrap/check-cluster-prereqs.sh`
- `platform/bootstrap/local-kind-not-used.md`
- `platform/certificates/README.md`
- `platform/certificates/cert-manager/README.md`
- `platform/certificates/cert-manager/cluster-issuer-banklab-ca.example.yaml`
- `platform/certificates/cert-manager/cluster-issuer-selfsigned.example.yaml`
- `platform/certificates/cert-manager/helm-values.yaml`
- `platform/certificates/cert-manager/kustomization.yaml`
- `platform/certificates/cert-manager/namespace.yaml`
- `platform/gitops/README.md`
- `platform/gitops/app-of-apps/README.md`
- `platform/gitops/app-of-apps/cert-manager-app.yaml`
- `platform/gitops/app-of-apps/kong-baseline-app.yaml`
- `platform/gitops/app-of-apps/metallb-app.yaml`
- `platform/gitops/app-of-apps/namespaces-app.yaml`
- `platform/gitops/app-of-apps/networking-app.yaml`
- `platform/gitops/app-of-apps/platform-prereqs-app.yaml`
- `platform/gitops/app-of-apps/root-app.yaml`
- `platform/gitops/app-of-apps/security-app.yaml`
- `platform/gitops/projects/README.md`
- `platform/gitops/projects/platform-project.yaml`
- `platform/gitops/projects/security-project.yaml`
- `platform/gitops/projects/tenant-project-template.yaml`
- `platform/identity/README.md`
- `platform/kong/README.md`
- `platform/kong/argocd/README.md`
- `platform/kong/argocd/kong-baseline-app.yaml`
- `platform/kong/argocd/kong-gateway-api-app.yaml`
- `platform/kong/argocd/kong-smoke-app.yaml`
- `platform/kong/gateway-api/README.md`
- `platform/kong/gateway-api/gateway-api-crds.README.md`
- `platform/kong/gateway-api/gateway-external.yaml`
- `platform/kong/gateway-api/gateway-internal.yaml`
- `platform/kong/gateway-api/gatewayclass-kong.yaml`
- `platform/kong/gateway-api/kustomization.yaml`
- `platform/kong/helm/README.md`
- `platform/kong/helm/render-kong-baseline.sh`
- `platform/kong/helm/values-kong-oss-baseline.schema.yaml`
- `platform/kong/helm/values-kong-oss-baseline.yaml`
- `platform/kong/kustomization.yaml`
- `platform/kong/namespace.yaml`
- `platform/kong/network-policies/README.md`
- `platform/kong/network-policies/kong-allow-dns.yaml`
- `platform/kong/network-policies/kong-allow-kube-api-placeholder.yaml`
- `platform/kong/network-policies/kong-allow-proxy-ingress.yaml`
- `platform/kong/network-policies/kong-allow-smoke-upstream.yaml`
- `platform/kong/network-policies/kong-default-deny.yaml`
- `platform/kong/network-policies/kustomization.yaml`
- `platform/kong/scripts/README.md`
- `platform/kong/scripts/check-admin-not-exposed.sh`
- `platform/kong/scripts/check-kong-baseline.sh`
- `platform/kong/scripts/render-and-lint-kong.sh`
- `platform/kong/scripts/route-smoke.sh`
- `platform/kong/smoke/README.md`
- `platform/kong/smoke/deployment.yaml`
- `platform/kong/smoke/httproute-external.yaml`
- `platform/kong/smoke/httproute-internal.yaml`
- `platform/kong/smoke/kustomization.yaml`
- `platform/kong/smoke/namespace.yaml`
- `platform/kong/smoke/service.yaml`
- `platform/kong/versions.yaml`
- `platform/namespaces/README.md`
- `platform/namespaces/kustomization.yaml`
- `platform/namespaces/platform-gitops.yaml`
- `platform/namespaces/platform-identity.yaml`
- `platform/namespaces/platform-networking.yaml`
- `platform/namespaces/platform-observability.yaml`
- `platform/namespaces/platform-security.yaml`
- `platform/namespaces/platform-system.yaml`
- `platform/namespaces/synthetic-clients.yaml`
- `platform/namespaces/tenant-accounts.yaml`
- `platform/namespaces/tenant-cards.yaml`
- `platform/namespaces/tenant-customer-profile.yaml`
- `platform/namespaces/tenant-fraud.yaml`
- `platform/namespaces/tenant-open-banking.yaml`
- `platform/namespaces/tenant-payments.yaml`
- `platform/networking/README.md`
- `platform/networking/metallb/README.md`
- `platform/networking/metallb/helm-values.yaml`
- `platform/networking/metallb/ip-address-pool.example.yaml`
- `platform/networking/metallb/kustomization.yaml`
- `platform/networking/metallb/l2-advertisement.example.yaml`
- `platform/networking/metallb/namespace.yaml`
- `platform/networking/network-policies/README.md`
- `platform/networking/network-policies/allow-dns.yaml`
- `platform/networking/network-policies/allow-gitops-to-managed-namespaces.yaml`
- `platform/networking/network-policies/allow-ingress-controller-placeholder.yaml`
- `platform/networking/network-policies/allow-observability-scrape-placeholder.yaml`
- `platform/networking/network-policies/kustomization.yaml`
- `platform/networking/network-policies/platform-default-deny.yaml`
- `platform/networking/network-policies/tenants-default-deny.yaml`
- `platform/observability/README.md`
- `platform/security/README.md`
- `platform/security/policies/README.md`
- `platform/security/policies/namespace-labels.md`
- `platform/security/sops/.sops.yaml.example`
- `platform/security/sops/README.md`
- `platform/security/sops/encrypted-secret.example.yaml`
- `playbooks-old/yaml/argocd-apps/cnpg/immich-db/base/immich-app-secret.yaml`
- `playbooks/argocd/applications/database/cnpg/immich-db/base/immich-app-secret.yaml`
- `playbooks/argocd/applications/media/booklore/booklore-secret.yaml`
- `policies/README.md`
- `policies/conftest/README.md`
- `policies/conftest/placeholder.rego`
- `policies/examples/invalid-api-metadata.yaml`
- `policies/examples/valid-api-metadata.yaml`
- `reports/README.md`
- `reports/goal-000-summary.md`
- `reports/goal-001-summary.md`
- `reports/goal-002-summary.md`
- `requirements-dev.txt`
- `scripts/README.md`
- `scripts/check_kong_cluster.py`
- `scripts/cluster_smoke.py`
- `scripts/generate_evidence_report.py`
- `scripts/render_kong_baseline.py`
- `scripts/render_prereqs.py`
- `scripts/validate_kong_baseline.py`
- `scripts/validate_prereqs.py`
- `scripts/validate_repo.py`
- `scripts/validate_yaml.py`
- `soydocs/kong-bank-lab/README.md`
- `soydocs/kong-bank-lab/goals/README.md`
- `soydocs/kong-bank-lab/goals/goal-000-repo-foundation.md`
- `soydocs/kong-bank-lab/goals/goal-001-platform-prereqs.md`
- `soydocs/kong-bank-lab/goals/goal-002-kong-oss-baseline.md`
- `soydocs/kong-bank-lab/roadmap.md`
- `tenants/README.md`
- `tenants/tenant-template/README.md`
- `tests/README.md`
- `tests/fixtures/README.md`
- `tests/integration/README.md`
- `tests/policy/README.md`
- `tests/policy/test_goal_001_prereq_policies.py`
- `tests/policy/test_goal_002_kong_policy.py`
- `tests/policy/test_goal_002_no_forbidden_kong_resources.py`
- `tests/policy/test_policy_placeholders.py`
- `tests/unit/README.md`
- `tests/unit/test_goal_001_namespace_metadata.py`
- `tests/unit/test_goal_001_no_kong_resources.py`
- `tests/unit/test_goal_001_prereqs_structure.py`
- `tests/unit/test_goal_002_admin_api_private.py`
- `tests/unit/test_goal_002_kic_gateway_api_compatibility.py`
- `tests/unit/test_goal_002_kong_structure.py`
- `tests/unit/test_goal_002_kong_versions.py`
- `tests/unit/test_goal_002_no_enterprise_features.py`
- `tests/unit/test_repo_foundation.py`

## Kong Namespace

- `platform-kong` is declared for Kong runtime resources.
- `platform-kong-smoke` is declared for the platform-owned smoke backend.

## Kong Helm Baseline

- `kong/ingress` values define DB-less Kong OSS.
- Kong Gateway uses `kong:3.9.3`.
- KIC uses `kong/kubernetes-ingress-controller:3.5.10`.
- Admin API is ClusterIP-only.
- Proxy service is LoadBalancer for MetalLB.
- Helm chart compatibility: local render passed; cluster behavior not verified.

## Gateway API Baseline

- `GatewayClass/kong` uses `konghq.com/kic-gateway-controller`.
- Internal and external Gateways are declared separately.
- HTTP smoke routes attach to those Gateways.

## Smoke Backend

- `hashicorp/http-echo:1.0.0` is pinned.
- The backend returns `banklab-kong-smoke-ok`.
- Labels mark it as not a business API.

## Admin API Exposure Control

- Static validation checks Admin API is not externally exposed.
- Kong Manager, Portal, and Portal API are disabled.

## NetworkPolicy Baseline

- Kong default-deny, DNS, proxy ingress, smoke upstream, and API-server
  placeholder egress policies are defined.
- Enforcement still depends on the cluster CNI.

## Argo CD Integration

- Kong baseline app templates exist.
- Repo URLs remain `REPLACE_WITH_REPO_URL` placeholders until configured.

## Local Validation Result

pass

## Cluster Changes Performed

None.

## Cluster Verification

Runtime-verified on Kubernetes context `kubernetes-admin@cluster.local`.

- `platform-kong` and `platform-kong-smoke` namespaces are present.
- KIC and both Kong Gateway pods are Ready.
- `GatewayClass/kong` is Accepted.
- `Gateway/kong-external` and `Gateway/kong-internal` are Programmed at `192.168.20.22`.
- External and internal smoke routes return `banklab-kong-smoke-ok`.
- Unknown host routing returns Kong HTTP `404`.
- KIC remained Running after Kong CRDs were installed for controller cache sync.

## Secrets Created

Only KIC webhook TLS secrets were runtime-created:

- `banklab-kong-controller-validation-webhook-ca-keypair`
- `banklab-kong-controller-validation-webhook-keypair`

## Kong Resources Created

Kong baseline runtime resources were applied.

Kong CRDs are installed cluster-wide for KIC cache sync, but no `KongPlugin`,
`KongConsumer`, `KongClusterPlugin`, or business API resources were created in
goal 002.

## Enterprise Kong Features Used

None.

## Admin API Externally Exposed

No. Static validation passed, the Admin service remains ClusterIP-only, the
proxy LoadBalancer exposes only port 80, and `192.168.20.22:8444` refused the
connection.

## Known Limitations

- Kong CRDs are present because KIC requires them for stable cache startup.
- Historical KIC log entries show the missing-CRD retry loop before `4cfa4fe`;
  after CRD installation the pod remained Ready and `--since=5m` logs were empty.

## Ready For Next Goal

Yes; ask ChatGPT Pro for `goal-003-synthetic-bank-apis`.
